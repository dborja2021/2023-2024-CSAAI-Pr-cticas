...
//       peso = pipeRandomWeight; // Peso aleatorio para simular la distancia entre nodos
...
btnCNet.onclick = () => {
    ...
    document.getElementById("nodos").innerHTML = numNodos + " nodos";
    document.getElementById("red").innerHTML = "Red generada";

  }

  btnMinPath.onclick = () => {
    document.getElementById("red").innerHTML = "La red no está generada. Por favor generar primero la red.";
    ...
    for (const nodo of rutaMinimaConRetardos.rutaMinima){
      nodo.color = 'green';
      tiempoTotal += nodo.delay/1000;
    }
   
    console.log("Ruta mínima con retrasos:", rutaMinimaConRetardos);
    console.log("Timepo total de retardo: ", tiempoTotal, "s");

    ctx.clearRect(0, 0, canvas.width, canvas.height);
    drawNet(redAleatoria);

    document.getElementById("tiempo").textContent = "Tiempo total: " + tiempoTotal.toFixed(4) + "s";
  }
